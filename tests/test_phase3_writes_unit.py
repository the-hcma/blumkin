...
def test_reminder_minutes_before_start() -> None:
    assert reminder_minutes_before_start("30m") == 30
    assert reminder_minutes_before_start("1d") == 1440
    with pytest.raises(ValueError, match="positive"):
        reminder_minutes_before_start("0m")
    with pytest.raises(ValueError, match="four-week maximum"):
        reminder_minutes_before_start("5w")
...
def test_calendar_create_records_a_compose_timestamp(monkeypatch) -> None:
    ...
    record_composed = MagicMock()
    monkeypatch.setattr("blumkin.skills.calendar_writes.record_composed", record_composed)
    asyncio.run(
        calendar_create(
            subject="Sync",
            start_raw="2026-08-26T11:00",
...
def test_calendar_update_teams_true_attaches(monkeypatch) -> None:
    ...
    payload = asyncio.run(calendar_update(event_id="evt-1", teams=True, tz_name="America/New_York"))
    assert payload["event"]["online_join_url"] == "https://teams.example/join"
    _pa = client.me.events.by_event_id.return_value.patch.await_args
...
def test_calendar_update_teams_false_removes(monkeypatch) -> None:
    updated = SimpleNamespace(
        id="evt-1",
        subject="Sync",
        start=None,
    ...
    assert _pa is not None
    patched = _pa.args[0]
    assert patched.is_online_meeting is False


def test_calendar_update_nothing_to_update_rejected(monkeypatch) -> None:
    monkeypatch.setattr(
        "blumkin.skills.calendar_writes.load_config",
        lambda: SimpleNamespace(default_tz="America/New_York", client_id="x"),
    )
    monkeypatch.setattr(
        "blumkin.skills.calendar_writes.create_graph_client", lambda _cfg: MagicMock()
    )
    from blumkin.skills.calendar_writes import calendar_update
...
def test_calendar_update_refetch_when_patch_returns_none(monkeypatch) -> None:
    ...
    payload = asyncio.run(
        calendar_update(event_id="evt-1", subject="Sync", tz_name="America/New_York")
    )
...
def test_mail_draft_subject_preserves_a_legacy_entity_without_a_semicolon(monkeypatch) -> None:
    # Review follow-up on #252: html.unescape also resolves ~150 legacy HTML4 names
    # that are valid *without* a trailing ";" (&copy, &amp, ...). Only well-formed,
    # ";"-terminated entities should be touched, or an ordinary pasted query string
    # like "...?n=1&copy=2" would be silently mangled to "...?n=1\xa92".
    draft = SimpleNamespace(id="draft-1", subject="link")
    client = MagicMock()
    client.me.messages.post = AsyncMock(return_value=draft)
    monkeypatch.setattr("blumkin.skills.mail.create_graph_client", lambda _cfg: client)
    monkeypatch.setattr(
    ...
    asyncio.run(
        mail_draft(
            to="a@b.com",
            subject="New link: https://x.io/a?n=1&copy=2",
            body="hi",
            body_type="text",
        )
    )
    post_await = client.me.messages.post.await_args
    assert post_await is not None
    assert post_await.args[0].subject == "New link: https://x.io/a?n=1&copy=2"


def test_mail_draft_rejects_a_subject_that_is_only_entities(monkeypatch) -> None:
    # Review follow-up on #252: the "--subject is required" check ran on the raw
    # value before un-escaping, so "&nbsp;" (whitespace once decoded) slipped
...
def test_mail_update_draft_rejects_a_subject_that_is_only_entities(monkeypatch) -> None:
    existing = SimpleNamespace(
        id="draft-1",
        is_draft=True,
        subject="Old",
