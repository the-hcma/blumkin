...
def test_chat_send_mocked(monkeypatch) -> None:
    ...
    created = SimpleNamespace(
        id="msg-9",
        body=SimpleNamespace(content="hello"),
    ...
    payload = asyncio.run(chat_send(with_name="daniel", text="  hello  "))
    assert payload["message"]["id"] == "msg-9"
    assert payload["message"]["body_text"] == "hello"
...
def test_chat_edit_mocked(monkeypatch) -> None:
    ...
    payload = asyncio.run(chat_edit(chat_id="chat-1", message_id="msg-1", text="edited"))
    assert payload["message"]["body_text"] == "edited"
...
def test_meeting_get_not_online_raises(monkeypatch) -> None:
    event = SimpleNamespace(
        id="evt-1",
