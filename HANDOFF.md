...
- [ ] **TODO (validate after grant):** add the **new** scopes to the Entra client, delete token cache + auth record, `blumkin auth login`, confirm consent includes at least `Chat.ReadWrite` and `OnlineMeetings.ReadWrite`, then smoke the blocked lab flows before implementing Blumkin Phase 4 skills.
